        ...
        {datas.map((data) => (
          ...
          <SwiperSlide key={data.id} className="pt-10 pb-5 ">
            {/* h-full is crucial here. It makes all visible slides stretch to the height
                        ...
                        */}
            <div
              className="relative px-8 pb-8 pt-14 
                            bg-blue-100/5 border border-blue-500/20 
                            rounded-xl backdrop-blur-sm transition-all duration-300 hover:border-blue-500/40 
                            hover:-translate-y-1 group h-full"
            >
              <div
                className="absolute -top-6 left-1/2 -translate-x-1/2 md:left-3 
              md:-translate-x-0 w-14 h-14 flex items-center justify-center 
              rounded-full shadow-lg bg-gradient-to-r from-[#061125] to-[#061125] border border-blue-500/20"
              >
                <Image
                  src={data.icon}
                  alt={data.title}
