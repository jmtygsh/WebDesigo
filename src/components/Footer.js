...
import { navItems, ourService, socialLinks, ourExpertise } from "@/data/index";
import { Button } from "@/components/Button";

export const Footer = () => {
    return (
        <footer className="relative border-t border-gray-800 py-12 px-4 md:px-8 lg:px-16 font-sans overflow-hidden">
            {/* Background gradient */}
            <div className='absolute top-0 left-0 w-full h-full bg-gradient-to-r from-blue-500/10 to-transparent z-0'></div>

            {/* Main content container */}
            <div className="relative z-10 max-w-7xl mx-auto">
                {/* KEY CHANGE: Main layout container.
                  - `flex-col` on mobile to stack the main sections.
                  - `lg:flex-row` on large screens to create the multi-column layout.
                  - `lg:items-start` to align items to the top on large screens.
                */}
                <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-12">

                    {/* Left Section: Logo, Socials, & CTA */}
                    <div className="flex flex-col items-center text-center lg:items-start lg:text-left lg:max-w-xs">
                        <Link href="/" aria-label="Home" className='z-20 text-xl font-semibold mb-4'>
                            WebDesigo
                        </Link>
                    ...
                    {/* KEY CHANGE: Right Section Link Lists.
                      - `grid` on mobile for a clean 2-column layout.
                      - `md:flex` on medium screens to spread them out.
                      - `flex-1` and `justify-between` to make them fill the space.
                    */}
                    <div className='flex-1 grid grid-cols-2 md:grid-cols-3 lg:flex lg:justify-end gap-8 md:gap-12'>
                        <div>
