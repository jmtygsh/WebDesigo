...
export const Count = ({ data }) => {
    ...
    return (
        <div className="grid grid-cols-2 gap-5">
            {data.map((item) => (
                <div key={item.id} className="relative bg-blue-100/5 border border-blue-500/20 rounded-xl p-8 backdrop-blur-sm transition-all duration-300 hover:border-blue-500/40 hover:-translate-y-0.5 hover:shadow-2xl hover:shadow-blue-500/15 group overflow-hidden">
